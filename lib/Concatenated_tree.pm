...
sub concatenated_tree { # create a tree for the concatenated genes
    ...
    my ($ml_tree,$ml_score) = PifCosm_support_subs::run_raxml ( 'XXXtemp.phy', $path, 'XXXtemp', $n_threads, $method . $method_append, $get_partitions);
            ...
            if ($method eq 'RAxML' or $method eq 'ExaML') {
                ...
                if ($store_boot_trees eq 'y') {
                    if (-e "RAxML_bootstrap.XXXtemp") {
                        open BOOTTREES, "<RAxML_bootstrap.XXXtemp" or die "Could not open file with bootstrap trees: $!.\n";
                        my $boot_trees = '';
    ...
    else { unlink ("XXXtemp.phy","XXXtemp.phy.reduced"); die "Could not construct any ML tree.\n"; }
